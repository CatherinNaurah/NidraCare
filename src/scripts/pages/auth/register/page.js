...
export default class Page {
    ...
    return `
      <section class="flex flex-col md:flex-row min-h-screen">
        ...
        <section class="md:w-1/2 flex items-center justify-center p-8" style="min-height: 480px">
          <form id="register-form" autocomplete="off" class="p-8 w-full max-w-md">
            <div class="mb-6 text-left">
              <h2 class="text-2xl font-bold text-black">Buat Akun Baru</h2>
              <p class="text-black/40 mt-2">Selamat datang di <strong>NidraCare</strong> — daftarkan diri Anda untuk mulai menggunakan layanan kami.</p>
            ...
            <div id="submit-button-container">
              <button type="submit" class="cursor-pointer w-full bg-[#0a1446] text-white font-semibold px-4 py-3 rounded-xl hover:bg-[#0c1a5a] transition">
                Daftar
              </button>
            </div>
            ...
            <p class="text-center text-sm mt-4 text-black">
              Sudah punya akun?
              <a href="#/login" class="font-semibold text-[#0a1446] hover:underline">Login Sekarang</a>
            </p>
          </form>
        </section>
      </section>
  ...
  showSubmitLoadingButton() {
    document.getElementById('submit-button-container').innerHTML = `
      <button class="cursor-pointer w-full bg-gray-500 text-white font-semibold px-4 py-3 rounded-xl" disabled>
